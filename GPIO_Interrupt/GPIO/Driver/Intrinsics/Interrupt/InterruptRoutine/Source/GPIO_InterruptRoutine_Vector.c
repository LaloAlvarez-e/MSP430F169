 ...
 */
#include "GPIO/Driver/Intrinsics/Interrupt/InterruptRoutine/Header/GPIO_InterruptRoutine_Vector.h"
...
__interrupt void PORT1_IRQVectorHandler(void)
{
    uint16_t (*IRQSourceHandlerReg)
            (PORT_EXT_t* pstPortArg, GPIO_nPIN_NUMBER enPinNumberArg) = (uint16_t (*)
                    (PORT_EXT_t* pstPortArg, GPIO_nPIN_NUMBER enPinNumberArg)) 0UL;
    uint16_t u16Status = LPM4_bits;
    uint8_t u8Flags = PORT1_IFG_R;
    uint8_t u8Enable = PORT1_IE_R;
    uintptr_t optrBaseAddress = 0U;

    u8Flags &= u8Enable;
    optrBaseAddress = GPIO__uptrBlockBaseAddress(GPIO_enPORT1);
    if(0U != (u8Flags & PORT_IFG_R_PIN0_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER0);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN1_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER1);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN2_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER2);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN3_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER3);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN4_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER4);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN5_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER5);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN6_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER6);
    }
    if(0U != (u8Flags & PORT_IFG_R_PIN7_MASK))
    {
        ...
        u16Status &= IRQSourceHandlerReg((PORT_EXT_t*) optrBaseAddress, GPIO_enPIN_NUMBER7);
    }
...
__interrupt void PORT2_IRQVectorHandler(void)
{
    ...
    u8Flags &= u8Enable;
    optrBaseAddress = GPIO__uptrBlockBaseAddress(GPIO_enPORT2);
