/**
 *
 * @file FLASH_Interval.c
 * @copyright
 * @verbatim InDeviceMex 2021 @endverbatim
 *
 * @par Responsibility
 * @verbatim InDeviceMex Developers @endverbatim
 *
 * @version
 * @verbatim 1.0 @endverbatim
 *
 * @date
 * @verbatim 26 feb. 2022 @endverbatim
 *
 * @author
 * @verbatim InDeviceMex @endverbatim
 *
 * @par Change History
 * @verbatim
 * Date           Author     Version     Description
 * 26 feb. 2022     InDeviceMex    1.0         initial Version@endverbatim
 */
#include "DriverLib/FLASH/Driver/Header/FLASH_Interval.h"

#include "DriverLib/FLASH/Driver/Intrinsics/Primitives/FLASH_Primitives.h"
#include "DriverLib/FLASH/Peripheral/FLASH_Peripheral.h"
#include "DriverLib/MCU/MCU.h"

void FLASH__vSetInterval(FLASH_nINTERVAL enIntervalArg)
{
    FLASH_Register16Bits_t pstRegisterData = {0UL};
    uint16_t u16Value = 0U;
    u16Value = (uint16_t) enIntervalArg;
    u16Value <<= FLASH_CTL_R_IS_BIT;
    u16Value |= FLASH_CTL_R_PW_WRITE;
    u16Value |= FLASH_CTL_R_CNTCL_CLEAR;
    pstRegisterData.uptrAddress = FLASH_CTL_OFFSET;
    pstRegisterData.u16Value = u16Value;
    pstRegisterData.u16Mask = FLASH_CTL_R_PW_MASK | FLASH_CTL_R_CNTCL_CLEAR | FLASH_CTL_R_IS_MASK;
    pstRegisterData.u8Shift = 0UL;

    FLASH__vWriteRegister_16bits(&pstRegisterData);
}

FLASH_nINTERVAL FLASH__enGetInterval(void)
{
    FLASH_Register16Bits_t pstRegisterData = {0UL};
    pstRegisterData.uptrAddress = FLASH_CTL_OFFSET;
    pstRegisterData.u16Value = (uint16_t) FLASH_enINTERVAL_32768;
    pstRegisterData.u16Mask = FLASH_CTL_IS_MASK;
    pstRegisterData.u8Shift = FLASH_CTL_R_IS_BIT;
    (void) FLASH__u16ReadRegister(&pstRegisterData);

    return ((FLASH_nINTERVAL) pstRegisterData.u16Value);
}

uint16_t FLASH__u16GetInterval(void)
{
    const uint16_t pu16LUTInterval[4UL] = {32768U, 8192U, 512U, 64U};
    FLASH_nINTERVAL enIntervalReg = FLASH_enINTERVAL_32768;
    uint16_t u16Reg = 0U;
    uint16_t u16Index = 0U;
    enIntervalReg = FLASH__enGetInterval();
    u16Index = (uint8_t) enIntervalReg;
    u16Reg = pu16LUTInterval[u16Index];
    return (u16Reg);
}



