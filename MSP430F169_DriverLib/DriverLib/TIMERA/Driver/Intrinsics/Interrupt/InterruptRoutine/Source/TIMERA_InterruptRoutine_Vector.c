...
__interrupt void TIMERA_CC0_IRQVectorHandler(void)
{
    ...
    if(TIMERA_CC_CTL_R_CAP_COMPARE == u16Mode)
    {
        ...
        u16Status &= IRQSourceHandlerReg(TIMERA_CC0_BASE, (uint8_t) TIMERA_enCC_MODE_COMPARE);
    }
    else
    {
        IRQSourceHandlerReg = TIMERA_CC__pu16fGetIRQSourceHandler(TIMERA_enCC0, TIMERA_enCC_MODE_CAPTURE);

        u16Status &= IRQSourceHandlerReg(TIMERA_CC0_BASE, (uint8_t) TIMERA_enCC_MODE_CAPTURE);
    }
...
__interrupt void TIMERA_IRQVectorHandler(void)
{
    ...
    case TIMERA_IV_R_IV_CC1:
        ...
        if(TIMERA_CC_CTL_R_CAP_COMPARE == u16Mode)
        {
            IRQSourceHandlerReg = TIMERA_CC__pu16fGetIRQSourceHandler(TIMERA_enCC1, TIMERA_enCC_MODE_COMPARE);

            u16Status &= IRQSourceHandlerReg(TIMERA_CC1_BASE, (uint8_t) TIMERA_enCC_MODE_COMPARE);
        }
        else
        {
            IRQSourceHandlerReg = TIMERA_CC__pu16fGetIRQSourceHandler(TIMERA_enCC1, TIMERA_enCC_MODE_CAPTURE);

            u16Status &= IRQSourceHandlerReg(TIMERA_CC1_BASE, (uint8_t) TIMERA_enCC_MODE_CAPTURE);
        }
        break;
    case TIMERA_IV_R_IV_CC2:
        ...
        if(TIMERA_CC_CTL_R_CAP_COMPARE == u16Mode)
        {
            ...
            u16Status &= IRQSourceHandlerReg(TIMERA_CC2_BASE, (uint8_t) TIMERA_enCC_MODE_COMPARE);
        }
        else
        {
            IRQSourceHandlerReg = TIMERA_CC__pu16fGetIRQSourceHandler(TIMERA_enCC2, TIMERA_enCC_MODE_CAPTURE);

            u16Status &= IRQSourceHandlerReg(TIMERA_CC2_BASE, (uint8_t) TIMERA_enCC_MODE_CAPTURE);
        }
        break;
    default:
