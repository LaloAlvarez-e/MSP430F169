...
uint16_t MAIN_u16DACCycleComplete(uintptr_t uptrBlock, uint16_t u16Conversion);
uint16_t MAIN_u16Timer(uintptr_t uptrBlock, uint16_t u16Conversion);
...
void MAIN_vTIMERA1Init(void)
{
    ...
    TIMERA_CC__vRegisterIRQSourceHandler(TIMERA_enCC1, TIMERA_enCC_MODE_COMPARE, &MAIN_u16Timer);
