        ...
        if(u8Previous!=u8Actual)
        {
            if(u8Actual==0)
            {
                ...
                if(u8Menu==0)
                {
                    ...
                    NOKIA5110__vClear();
                    u8Column=0, u8Row=0;
                    NOKIA5110__u16Print("InDev RTOS\n\rBoton 1: 000Boton 2: 000",&u8Column,&u8Row);
                    u8Column=0;
                    u8Row=3;
                ...
                if(u8Menu==1)
                {
                    ...
                    NOKIA5110__vClear();
                    u8Column=0, u8Row=0;
                    NOKIA5110__u16Print("InDev RTOS\n\rTemp: 000.00MAX6675",&u8Column,&u8Row);
                    OS__vSignalSemaphore(&MAIN_sSemaphoreNOKIA);
                    PBUTTON2_INT&=~PBUTTON2_READPIN;
                    PBUTTON1_INT&=~PBUTTON1_READPIN;
                    PBUTTON2_IFG&=~PBUTTON2_READPIN;
...
void Task4 (void)
{
    ...
    while(1)
    {
        if(MAIN_u8Menu==1)
        {
            ...
            OS__vWaitSemaphore(&MAIN_sSemaphoreNOKIA);
            NOKIA5110__u8SendString("Stack:",&u8Column,&u8Row);
            u8Column=8;
            NOKIA5110__u8SendString((char*)TASK4_cConv,&u8Column,&u8Row);
            NOKIA5110__u8SendString("%",&u8Column,&u8Row);
            OS__vSignalSemaphore(&MAIN_sSemaphoreNOKIA);
